class Solution:
    def search(self,nums, target: int) -> int:   #in question there a list which is sorted 
        ...
        while low <= high:
          ...
          # when 1st step not satisfied then we run elif part where mid index value lesser than target we increas the value
             ...
             #when previous steps not satisfied then we run else part where mid index value greater than target we reduce the value
                ...
                high = mid - 1
# if there all steps fail that mean here not have our targeted value so we return -1 which show not found our targeted value
        return -1